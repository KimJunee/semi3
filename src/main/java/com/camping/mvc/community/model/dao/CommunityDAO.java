...
public class CommunityDAO {
	...
	public List<Community> findAll(Connection conn, PageInfo pageInfo) {
		...
		String query = "SELECT RNUM, CO_NO,CO_TITLE,USER_ID,CO_REGIST, CO_HIT " + "FROM ( "
				+ "    SELECT ROWNUM AS RNUM, CO_NO,CO_TITLE,USER_ID,CO_REGIST, CO_HIT " + "    FROM ( "
				+ "        SELECT CB.CO_NO,CB.CO_TITLE,UT.USER_ID,CB.CO_REGIST, CB.CO_HIT "
				+ "        FROM COMMU_BOARD CB JOIN USER_T UT ON(CB.USER_NO = UT.USER_NO) "
				+ "         ORDER BY CB.CO_NO " + "    ) " + ") " + "WHERE RNUM BETWEEN ? and ?";
	...
	public int getCommunityCount(Connection conn, String searchValue) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String query = "SELECT  COUNT(*) " + "FROM COMMU_BOARD CB " + "JOIN USER_T UT ON(CB.USER_NO = UT.USER_NO) "
				+ "WHERE 1=1 ";
	...
	public List<Community> findAll(Connection conn, PageInfo pageInfo, String searchValue) {
		...
		ResultSet rs = null;
		List<Community> list = new ArrayList<Community>();
		String queryBefore = "SELECT RNUM, CO_NO,USER_ID, CO_TITLE,CO_CONTENT,CO_REGIST,CO_HIT " + "FROM ( "
				+ "    SELECT ROWNUM AS RNUM,CO_NO,USER_ID, CO_TITLE,CO_CONTENT,CO_REGIST,CO_HIT " + "    FROM ( "
				+ "        SELECT  CB.CO_NO,UT.USER_ID, CB.CO_TITLE,CB.CO_CONTENT,CB.CO_REGIST,CB.CO_HIT "
				+ "        FROM COMMU_BOARD CB JOIN USER_T UT ON(CB.USER_NO = UT.USER_NO) " + "        WHERE 1 = 1 ";
	...
	public Community findBoardByNo(Connection conn, int communityNo) {
		...
		String query = "SELECT  CB.CO_NO, UT.USER_ID, CB.CO_TITLE, CB.CO_CONTENT, CB.CO_REGIST, CB.CO_HIT "
				+ "FROM COMMU_BOARD CB " + "JOIN USER_T UT ON(CB.USER_NO = UT.USER_NO) " + "WHERE CB.CO_NO=?";
		try {
			pstmt = conn.prepareStatement(query);
	...
	private List<Reply> getRepliesByNO(Connection conn, int communityNo) {
		...
		String query = "SELECT R.REP_NO, R.CO_NO,R.REP_CONTENT, UT.USER_ID, R.REP_REGIST " + "FROM REPLY R "
				+ "JOIN USER_T UT  ON(R.USER_NO = UT.USER_NO) " + "WHERE  R.CO_NO= ? " + "ORDER BY R.REP_NO DESC ";

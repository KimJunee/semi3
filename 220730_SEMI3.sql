...
BEGIN
    FOR N IN 1..25
    LOOP
        INSERT INTO INQUIRY_BOARD VALUES(SEQ_INQUIRY_BOARD_INQUIRY_NO.NEXTVAL,1, '문의글 제목'||SEQ_INQUIRY_BOARD_INQUIRY_NO.NEXTVAL,'문의글 내용'||SEQ_INQUIRY_BOARD_INQUIRY_NO.NEXTVAL,SYSDATE, 1 );
    END LOOP;
...
INSERT INTO INQUIRY_BOARD VALUES(SEQ_INQUIRY_BOARD_INQUIRY_NO.NEXTVAL,1, '문의글 SQL테스트 제목입니다1','문의글 SQL 테스트 내용입니다1',SYSDATE, 1);
INSERT INTO INQUIRY_BOARD VALUES(SEQ_INQUIRY_BOARD_INQUIRY_NO.NEXTVAL,2, '문의글 SQL테스트 제목입니다2','문의글 SQL 테스트 내용입니다2',SYSDATE, 1);
...
-- 문의글 작성(확인완)
INSERT INTO INQUIRY_BOARD VALUES(SEQ_INQUIRY_BOARD_INQUIRY_NO.NEXTVAL,1,'테스트제목 1','테스트내용2',DEFAULT,DEFAULT);

-- 문의글 수정_마이페이지(확인완_안씀)
UPDATE INQUIRY_BOARD SET INQUIR_TITLE=?,INQUIR_CONTENT=? WHERE INQUIRY_NO=?;

-- 문의글 삭제_마이페이지(확인완)
DELETE INQUIRY_BOARD  WHERE INQUIRY_NO=28;
...
--게시판 검색기능(확인완)
SELECT RNUM, INQUIRY_NO,USER_ID,INQUIR_TITLE,INQUIR_CONTENT,INQUIR_REGIST,INQUIR_HIT  
FROM (
...
CREATE TABLE INREPLY(
    ...
    FOREIGN KEY(USER_NO) REFERENCES  USER_T
    ON DELETE CASCADE ,     --작성자번호 외래키
...
-- 캠핑장 리뷰 리스트 가져오는 쿼리문 
SELECT CS_NO, U.USER_NO, U.USER_ID, REV_NO, REV_TITLE, REV_CONTENT, REV_REGIST, REV_IMAGE, REV_IMAGE_RENAME, REV_STAR 
...
-----------------예약 DML---------------------------------------------------------------
-- 예약하기
-- INSERT INTO RESERVATION VALUES (SEQ_RES_NO.NEXTVAL, ?, ?, ?, ?, ?, ?, DEFAULT);
 
-- 예약 조회하기
-- SELECT RESV_NO, USER_NO, CS.CS_NAME, CS.CS_NO, CS_IMAGE, CS.CS_INDUTY, 
...
-- 예약 상세조회
-- R.USER_NO, RESV_NO, RESV_CHECKIN, RESV_CHECKOUT, RESV_HEADCOUNT,
